...
_CacheCursor.getSavedCacheCursorPath = (userId) => {
  const baseDir = path2.resolve("cache_cursor", userId);
  return {
    POSTS_ID_CACHE_CURSOR_PATH: path2.resolve(baseDir, "posts_id.json")
  };
};
...
PathUtils.getSavedProfileStatisticsDirPath = (userId) => {
  ...
  return {
    LIKED_PAGES: path3.resolve(BASE_DIR, "liked_paged.csv"),
    FRIENDS: path3.resolve(BASE_DIR, "friends.csv"),
    FOLLOWING: path3.resolve(BASE_DIR, "following.csv"),
    JOINED_GROUPS: path3.resolve(BASE_DIR, "joined_groups.csv"),
    SENT_FRIEND_REQUESTS: path3.resolve(BASE_DIR, "sent_friend_requests.csv")
  };
};
...
var FacebookCleaner = class {
  constructor(cookies) {
    ...
    this.deleteReactions = async (fromDate, toDate, batchSize = 5) => {
      ...
      for (let i = startDate; i.isSameOrBefore(endDate); i = dayjs2(i).add(1, "month")) {
