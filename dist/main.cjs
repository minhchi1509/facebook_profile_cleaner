...
class ObjectUtils {
    ...
    static findByKey = ({ data, key, condition, }) => {
...
class FacebookRequest {
    ...
    executeWithRetry = async ({ action, actionName, }) => {
        ...
        try {
            for (let attempt = 0; attempt < maxRetries; attempt += 1) {
                try {
                    ...
                    console.warn(`⚠️ ${actionName} failed (attempt ${attempt + 1}/${maxRetries}). Retrying in ${delayBetweenRetries}ms...`);
    ...
    getProfileCredentials = async () => {
        try {
            const { data } = await this.axiosInstance.get("https://www.facebook.com/");
    ...
    getProfileTabKey = async (profileCredentials) => {
        try {
            ...
            throw new Error("❌ Error when getting profile tab key: " +
                (error instanceof Error ? error.message : "Unknown error"));
        }
    };
    makeRequestToFacebook = async ({ docID, query, profileCredentials, headers, }) => {
    ...
    getFollowedPages = async ({ cursor, profileCredentials, }) => {
        ...
        if (!originalLikedPagesList || !pageInfor) {
            throw new Error("❌ Error when getting liked pages: Response data is missing expected fields.");
        }
    ...
    getFriends = async ({ cursor, profileCredentials, profileTabKey, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        ...
        if (!originalFriendsList || !pageInfor) {
            throw new Error("❌ Error when getting friends: Response data is missing expected fields.");
        }
    ...
    getFollowing = async ({ cursor, profileCredentials, profileTabKey, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        const tabKey = profileTabKey || (await this.getProfileTabKey(credentials));
        const query = {
        ...
        if (!originalFollowingList || !pageInfor) {
            throw new Error("❌ Error when getting following: Response data is missing expected fields.");
        }
    ...
    getJoinedGroups = async ({ cursor, profileCredentials, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        const query = {
        ...
        if (!originalJoinedGroupsList || !pageInfor) {
            throw new Error("❌ Error when getting joined groups: Response data is missing expected fields.");
        }
    ...
    getSentFriendRequests = async ({ cursor, profileCredentials, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        const query = {
            scale: 1,
            count: 10,
        ...
        const pageInfor = responseData?.data?.viewer?.outgoing_friend_requests_connection
            ?.page_info;
        if (!originalSentFriendRequests || !pageInfor) {
            throw new Error("❌ Error when getting sent friend requests: Response data is missing expected fields.");
        }
    ...
    getPostsData = async ({ cursor, profileCredentials, month, year, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        const query = {
        ...
        const originalPostsData = responseData?.data?.viewer?.activity_log_actor?.activity_log_stories
            ?.edges;
        const pageInfor = responseData?.data?.viewer?.activity_log_actor?.activity_log_stories
            ?.page_info;
    ...
    getReactionData = async ({ cursor, profileCredentials, month, year, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        const query = {
            activity_history: false,
            audience: null,
        ...
        const originalReactionsData = responseData?.data?.viewer?.activity_log_actor?.activity_log_stories
            ?.edges;
        const pageInfor = responseData?.data?.viewer?.activity_log_actor?.activity_log_stories
            ?.page_info;
    ...
    getCommentData = async ({ cursor, profileCredentials, month, year, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        const query = {
            activity_history: false,
            audience: null,
        ...
        const originalCommentsData = responseData?.data?.viewer?.activity_log_actor?.activity_log_stories
            ?.edges;
        const pageInfor = responseData?.data?.viewer?.activity_log_actor?.activity_log_stories
            ?.page_info;
    ...
    unfriend = async ({ profileCredentials, friendId, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        const responseData = await this.makeRequestToFacebook({
            profileCredentials: credentials,
    ...
    unfollowPage = async ({ profileCredentials, pageId, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
    ...
    leaveGroup = async ({ profileCredentials, groupId, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
    ...
    unfollowUserOrPage = async ({ profileCredentials, userOrPageId, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        ...
        if (!responseData?.data?.actor_unsubscribe?.unsubscribee?.id) {
            throw new Error(`❌ Error when unfollowing user or page with ID ${userOrPageId}.`);
        }
    };
    cancelSentFriendRequest = async ({ profileCredentials, userId, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        ...
        if (!responseData?.data?.friend_request_cancel?.cancelled_friend_requestee?.id) {
            throw new Error(`❌ Error when canceling sent friend request to user with ID ${userId}.`);
        }
    };
    deletePost = async ({ profileCredentials, postData, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        ...
        if (!responseData?.data?.activity_log_story_curation?.success) {
            throw new Error(`❌ Error when deleting post with ID ${postData.postId}.`);
        }
    };
    changePostPrivacy = async ({ profileCredentials, postId, privacy, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        const { userId } = credentials;
        ...
        if (!responseData?.data?.privacy_selector_save?.privacy_scope
            ?.privacy_scope_renderer?.id) {
            throw new Error(`❌ Error when changing privacy of post with ID ${postId}.`);
        }
    };
    deleteReaction = async ({ profileCredentials, reactionData, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        const docID = "24411931498505270";
        const query = {
            input: {
        ...
        if (!responseData?.data?.activity_log_story_curation?.success) {
            throw new Error(`❌ Error when deleting reaction with story ID ${reactionData.storyId} and post ID ${reactionData.postId}.`);
        }
    };
    deleteComment = async ({ profileCredentials, commentData, }) => {
        const credentials = profileCredentials || (await this.getProfileCredentials());
        const docID = "24411931498505270";
        const query = {
            input: {
        ...
        if (!responseData?.data?.activity_log_story_curation?.success) {
            throw new Error(`❌ Error when deleting comment with story ID ${commentData.storyId} and post ID ${commentData.postId}.`);
        }
    };
}
...
class FacebookCleaner {
    ...
    normalizeCleanerOptions = (options, actionName) => {
        ...
        if (!Number.isInteger(concurrencySize) || concurrencySize <= 0) {
            throw new Error(`❌ Invalid concurrencySize in ${actionName}: concurrencySize must be a positive integer.`);
        }
        if (!Number.isFinite(limit) && limit !== Number.POSITIVE_INFINITY) {
            throw new Error(`❌ Invalid limit in ${actionName}: limit must be a finite number or Infinity.`);
        }
        if (limit < 0) {
            throw new Error(`❌ Invalid limit in ${actionName}: limit must be greater than or equal to 0.`);
        }
        if (!Number.isFinite(delayInMs) || delayInMs < 0) {
            throw new Error(`❌ Invalid delayInMs in ${actionName}: delayInMs must be greater than or equal to 0.`);
        }
        return {
            concurrencySize,
            limit,
    ...
    fetchAllByCursor = async ({ limit, actionName, fetchPage, }) => {
        const allItems = [];
        let cursor = "";
        const targetText = Number.isFinite(limit) ? String(limit) : "unlimited";
        console.log(`📥 ${actionName}: Starting fetching items (target: ${targetText}).`);
        while (true) {
        ...
        console.log(`✅ ${actionName}: Completed fetching ${allItems.length} items.`);
        return allItems;
    };
    runBatchedActions = async ({ items, options, actionName, runAction, }) => {
        ...
        for (let startIndex = 0; startIndex < items.length; startIndex += options.concurrencySize) {
            const currentBatch = items.slice(startIndex, startIndex + options.concurrencySize);
            await Promise.all(currentBatch.map((item) => runAction(item)));
            processedItems += currentBatch.length;
            console.log(`🏃‍♂️‍➡️ ${actionName}: Processed ${processedItems}/${totalItems} items...`);
            if (processedItems < totalItems) {
                await this.delay(options.delayInMs);
            }
        }
        console.log(`✅ ${actionName}: Completed ${processedItems}/${totalItems} items.`);
    };
    resolveDateWindows = (fromDate, toDate) => {
        const hasFromDate = !!fromDate?.trim();
        const hasToDate = !!toDate?.trim();
        if (hasFromDate !== hasToDate) {
            throw new Error("❌ Invalid date input. Both fromDate and toDate are required when filtering by date.");
        }
        if (!hasFromDate && !hasToDate) {
        ...
        for (let currentDate = startDate; currentDate.isSameOrBefore(endDate); currentDate = dayjs(currentDate).add(1, "month")) {
    ...
    removeFriends = async (options = {}) => {
        const normalizedOptions = this.normalizeCleanerOptions(options, "removeFriends");
    ...
    unfollowPages = async (options = {}) => {
        const normalizedOptions = this.normalizeCleanerOptions(options, "unfollowPages");
        const profileCredentials = await this.facebookRequest.getProfileCredentials();
    ...
    leaveGroups = async (options = {}) => {
        const normalizedOptions = this.normalizeCleanerOptions(options, "leaveGroups");
        const profileCredentials = await this.facebookRequest.getProfileCredentials();
    ...
    unfollowUsersOrPages = async (options = {}) => {
        const normalizedOptions = this.normalizeCleanerOptions(options, "unfollowUsersOrPages");
        const { profileCredentials, profileTabKey } = await this.getCredentialsAndTabKey();
    ...
    cancelSentFriendRequests = async (options = {}) => {
        const normalizedOptions = this.normalizeCleanerOptions(options, "cancelSentFriendRequests");
        const profileCredentials = await this.facebookRequest.getProfileCredentials();
    ...
    deletePosts = async ({ fromDate, toDate, ...options }) => {
        const normalizedOptions = this.normalizeCleanerOptions(options, "deletePosts");
        const profileCredentials = await this.facebookRequest.getProfileCredentials();
        ...
        if (Number.isFinite(normalizedOptions.limit)) {
            console.log(`✅ DeletePosts overall: ${totalDeleted}/${normalizedOptions.limit}.`);
        }
        else {
            console.log(`✅ DeletePosts overall: ${totalDeleted} item(s).`);
        }
    };
    changePostsPrivacy = async ({ privacy, ...options }) => {
        const normalizedOptions = this.normalizeCleanerOptions(options, "changePostsPrivacy");
        const profileCredentials = await this.facebookRequest.getProfileCredentials();
    ...
    deleteReactions = async ({ fromDate, toDate, ...options }) => {
        const normalizedOptions = this.normalizeCleanerOptions(options, "deleteReactions");
        const profileCredentials = await this.facebookRequest.getProfileCredentials();
        const dateWindows = this.resolveDateWindows(fromDate, toDate);
        let remainingLimit = normalizedOptions.limit;
        let totalDeleted = 0;
        ...
        if (Number.isFinite(normalizedOptions.limit)) {
            console.log(`✅ DeleteReactions overall: ${totalDeleted}/${normalizedOptions.limit}.`);
        }
        else {
            console.log(`✅ DeleteReactions overall: ${totalDeleted} item(s).`);
        }
    };
    deleteComments = async ({ fromDate, toDate, ...options }) => {
        const normalizedOptions = this.normalizeCleanerOptions(options, "deleteComments");
        const profileCredentials = await this.facebookRequest.getProfileCredentials();
        const dateWindows = this.resolveDateWindows(fromDate, toDate);
        let remainingLimit = normalizedOptions.limit;
        let totalDeleted = 0;
        ...
        if (Number.isFinite(normalizedOptions.limit)) {
            console.log(`✅ DeleteComments overall: ${totalDeleted}/${normalizedOptions.limit}.`);
        }
        else {
...
class FileUtils {
    ...
    static writeCSV = async (filePath, data, includeHeader = true) => {
        ...
        return new Promise((resolve, reject) => {
            fastCsv.write(data, { headers: includeHeader })
                .pipe(writeStream)
                .on("finish", resolve)
                .on("error", reject);
        });
...
class FacebookStatistics {
    ...
    normalizeStatisticOptions = (options, statisticName) => {
        ...
        const limit = options?.limit ?? Number.POSITIVE_INFINITY;
        if (!Number.isFinite(limit) && limit !== Number.POSITIVE_INFINITY) {
            throw new Error(`❌ Invalid limit in ${statisticName}: limit must be a number`);
        }
        if (limit < 0) {
            throw new Error(`❌ Invalid limit in ${statisticName}: limit must be greater than or equal to 0`);
        }
        if (writeToCSV && !savedFilePath) {
            throw new Error(`❌ Invalid options in ${statisticName}: savedFilePath is required when writeToCSV is true`);
        }
        if (savedFilePath && !this.isValidSavedFilePath(savedFilePath)) {
            throw new Error(`❌ Invalid savedFilePath in ${statisticName}: ${savedFilePath}`);
        }
        return {
            writeToCSV,
            savedFilePath,
    ...
    fetchAllByCursor = async ({ options, statisticName, fetchPage, needProfileTabKey = false, }) => {
        const normalizedOptions = this.normalizeStatisticOptions(options, statisticName);
        const profileCredentials = await this.facebookRequest.getProfileCredentials();
        ...
        while (true) {
            ...
            console.log(`🚀 Fetched ${allItems.length} items from ${statisticName}...`);
            if (allItems.length >= normalizedOptions.limit ||
                !response.pagination.hasMore) {
                break;
            }
            cursor = response.pagination.nextCursor;
        }
        console.log(`✅ Fetched total ${allItems.length} items from ${statisticName} successfully.`);
